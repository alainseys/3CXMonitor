...
namespace PABXMonitor.Samples
{
    internal static class DisplayAllSample
    {
        public static void Run()
        {
            ...
            System.Console.WriteLine("SIPTrunks:");
            foreach (var e in ps.GetExternalLines())
            {
                //Find the name from our little extension cache

                System.Console.WriteLine(string.Format("\t{0}:{1}:{2}:{3}:{4}:{5}",
                                                            e.IsRegistered,
                                                            e.Number,
                                                            e.Direction,
                                                            string.Join("#", e.DIDNumbers),
                                                            e.ExternalNumber,
                                                            e.ID
                                                            ));
            }
            ...
            foreach (Tenant t in ps.GetTenants())
            {
                ...
                foreach (Extension e in t.GetExtensions())
                {
                    ...
                    System.Console.WriteLine(string.Format("{0}:{1}:{2}:{3}:{4}:{5}:{6}",
                                             e.Enabled,
                                             e.IsRegistered,
                                             e.Number,
                                             e.UserStatus,
